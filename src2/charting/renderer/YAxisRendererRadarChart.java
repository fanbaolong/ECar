	...
	protected void computeAxisValues(float min, float max)
	{
		...
		} else
		{
			...
			} else
			{
				...
				{
					// Force the first label to be at the 0 (or smallest
					// negative value)
					first = yMin;
				}

				if (first == 0.0) // Fix for IEEE negative zero case (Where
									// value == -0.0, and 0.0 == -0.0)
					first = 0.0;
		...
		{
			// If startAtZero is disabled, and the first label is lower that the
			// axis minimum,
