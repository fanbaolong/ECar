...
public class BarChartRenderer extends DataRenderer
{
	...
	public void initBuffers()
	{
		...
		for (int i = 0; i < mBarBuffers.length; i++)
		{
			...
			mBarBuffers[i] = new BarBuffer(set.getValueCount() * 4 * set.getStackSize(), barData.getGroupSpace(), barData.getDataSetCount(), set.isStacked());
		}
	}
			...
			for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++)
			{
				...
				if (!dataSet.isStacked())
				{
					...
					for (int j = 0; j < valuePoints.length * mAnimator.getPhaseX(); j += 2)
					{
						...
						drawValue(c, dataSet.getValueFormatter(), val, entry, i, valuePoints[j], valuePoints[j + 1] + (val >= 0 ? posOffset : negOffset));
					}
				...
				} else
				{
					...
					for (int j = 0; j < (valuePoints.length - 1) * mAnimator.getPhaseX(); j += 2)
					{
						...
						if (vals == null)
						{
							...
							drawValue(c, dataSet.getValueFormatter(), entry.getVal(), entry, i, valuePoints[j], valuePoints[j + 1] + (entry.getVal() >= 0 ? posOffset : negOffset));
	...
	{
		return trans.generateTransformedValuesBarChart(entries, dataSetIndex, mChart.getBarData(), mAnimator.getPhaseY());
	}

	protected boolean passesCheck()
	{
		return mChart.getBarData().getYValCount() < mChart.getMaxVisibleCount() * mViewPortHandler.getScaleX();
	}
